...
You've built a corporate financial model. Revenue flows from drivers. Costs build from components. The three statements link and balance. Scenarios stress-test the business. The DRIVERS framework has done its job.

Now what?

Now you wrap a transaction around it.

The corporate model tells you what the business does — how it generates revenue, incurs costs, produces cash flow. The LBO model tells you what happens when someone *buys* that business with borrowed money.

This isn't a small distinction. It's a transformation.
...
Here's something that trips up junior analysts: you can't build the LBO model until you've built the corporate model.
...
Here's what makes LBO analysis genuinely interesting: you're serving two masters.
...
This is what we covered in the corporate modeling chapter. Credit metrics. The 50% rule. Stress testing. The lender doesn't care about equity returns — they care about not losing money.
...
- What's my return on equity? (IRR)
- How much money do I get back? (Multiple of Capital, or MoC)
- How sensitive is my return to assumptions? (Entry/exit multiple sensitivity)
- What levers can I pull to improve returns? (Value creation bridges)
...
Here's the thing: these perspectives are linked but not aligned.
...
When a deal "works," it means the lender is comfortable with the credit risk *and* the sponsor is satisfied with the expected return. When you can't find that overlap, the deal doesn't happen.

### What We're Building

This chapter takes the Europa Packaging corporate model and transforms it into a complete LBO model. We'll add:
...
Olympus Partners, a mid-market European private equity fund, is acquiring Europa Packaging from its current owner (let's say a family who built the business over three generations and is now ready to monetise).
...
For Europa, we've assumed modest debt-like items: a small pension deficit (€10m), some warranty provisions (€5m), and derivative liabilities (€5m). These net out to €20m of adjustments.
...
The "Uses" side is simpler, so let's start there. What are we spending money on?
...
Let's examine the capital structure:
...
We'll refine these numbers, but the rough math works. The deal is feasible.
...
This is acquisition accounting. It's governed by IFRS 3 (or ASC 805 in the US), and it's surprisingly complicated. We'll cover the essentials.
...
The balance sheet balances. €654m = €654m. We're ready to project forward.
...
For our model, we'll assume Europa has no material NOLs — it's been consistently profitable. But in turnaround situations or acquisitions of distressed assets, NOL limitations can materially impact the deal economics.
...
In the corporate model chapter, we covered debt schedules at a conceptual level. Now let's build them properly for an LBO.
...
**Interest at EURIBOR + 450bps:** We're assuming EURIBOR stays at ~3.5%, giving an all-in rate of 8.0%. In practice, you might model a forward curve.
...
Let's see where the returns come from:
...
When the credit committee asks "what if EBITDA falls 20%?", you have an answer. When the investment committee asks "what's the breakeven entry multiple?", you have an answer. When someone challenges an assumption, you can change it and immediately see the implications.
...
*This completes the core modeling section of the book. The corporate model taught you to project a business. The LBO model taught you to wrap a transaction around it. In the chapters that follow, we'll apply these tools to increasingly complex situations — distressed credits, covenant negotiations, restructurings. But the foundation is now in place: you can model a business, structure a transaction, and evaluate it from both sides of the capital structure.*
