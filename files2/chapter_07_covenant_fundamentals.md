...
Five points consolidate the analytical framework that the remainder of this Part will develop in detail.

**1. The structure is the substance.** Detail-level analysis is only meaningful once the architecture is understood. The first questions on any covenant package are: incurrence or maintenance? What is the Restricted Group perimeter? Where does economic value sit relative to that perimeter?

**2. Cov-lite does not mean no covenants.** "Cov-lite" describes the absence of maintenance covenants. The incurrence covenants remain, and in a cov-lite structure they are the only ongoing contractual constraint on borrower behaviour. Their importance is therefore greater, not lesser, than in a covenanted structure.

**3. The Restricted Group is dispositive.** Value inside the Restricted Group is subject to the covenant package and is available to creditors. Value outside the Restricted Group is not. Sponsor counsel are systematically attentive to this distinction; creditor analysis must be equally attentive.

**4. The definitions section governs the operative provisions.** A covenant referring to a "Leverage Ratio" delegates substantive content to the definitions of Consolidated EBITDA, Consolidated Total Debt, and the permitted adjustments thereto. Definitional drafting determines the practical scope of every operative provision and is where most of the meaningful negotiation occurs.

**5. Covenant analysis is worst-case analysis.** Every basket, every carveout, and every definitional latitude in a covenant package will, in practice, be used to its outer limit by the borrower and its sponsor. Analysis that assumes ordinary-course utilisation will systematically understate the risks embedded in the package.
