...
A critical distinction bears emphasis from the outset.

A common misconception in leveraged finance is that modeling mastery constitutes the entire analytical skill set. This perspective is incomplete.

Practitioners who succeed in leveraged finance distinguish themselves not through modeling sophistication but through understanding a model's purpose and limitations. They recognize when a model serves analytical thinking and when it constrains it.

It is not to suggest that modeling is immaterial. Mastery of the mechanics — three-statement integration, credit ratios, scenario analysis, decision frameworks — is essential. Without the ability to build a model, the job cannot be performed.
...
This chapter provides detailed technical instruction in constructing a corporate financial model from first principles. By the chapter's conclusion, the reader will be equipped to build a model that captures a business's operational drivers and stress-tests capital structure sustainability.

The reader should maintain perspective: a model is a simplified representation, not the phenomenon itself. Europa Packaging — our case study — exists in the real world, with real factories and real customers and real competitive pressures. The model is a simplified representation of that reality. Useful, but incomplete.

The goal isn't to build impressive models. The goal is to build *useful* models — and to know the difference.

### What This Chapter Covers

This chapter teaches how to construct a corporate financial model from an operational and business perspective. The approach begins with the company as it exists: its revenue drivers, cost structure, working capital dynamics, and capital expenditure needs. These elements are then projected forward, stress-tested under different scenarios, and assessed for sufficiency in generating cash to service debt.

This foundation is essential. It precedes any analysis of leveraged transactions.

The subsequent chapter builds on this foundation by adding the complexity of an LBO: a new capital structure, acquisition accounting adjustments, and sponsor economics. The LBO model is constructed *on top of* the corporate model. A flawed corporate model will introduce corresponding flaws into the LBO model.
...
The most impressive financial models often contain not a single complex formula.

This runs counter to intuition. One might expect sophisticated analysis to require sophisticated machinery — nested IF statements, array formulas, VBA wizardry. Intuitively, a model handling a €500m leveraged buyout should be more elaborate than one tracking a household budget.
...
A model built today will be used by others tomorrow. That someone might be a colleague providing coverage during an absence. It might be a credit committee member stress-testing an assumption. It might be the original builder, six months from now, having forgotten the original design rationale.
...
Before addressing frameworks, formulas, debt schedules, working capital, and tax shields, the discussion turns to building something that endures. Models should work not just today, but indefinitely. They should be usable not just by the original builder, but by everyone who encounters them.
...
A revenue forecast with 3% growth is being constructed. The formula is written as:

```
Revenue (FY25) = Revenue (FY24) * 1.03
```

The model produces the expected output. Six months later, a request arrives to examine 2% growth. Now every cell containing "1.03" must be found and changed to "1.02". A single missed cell creates internal inconsistency; multiple misses create chaos.

The solution requires discipline: every assumption must reside in a dedicated cell, clearly labeled, in a dedicated section. The formula becomes:
...
This seems elementary, but discipline frequently lapses. Major investment banks have produced models — deployed to commit hundreds of millions of euros — riddled with hard-coded numbers buried in formulas. The analysts responsible were skilled and capable. The errors arose from momentary shortcuts, repeated, and eventually embedded as habit.
...
Merged cells are tempting because they produce visual appeal — headers spanning multiple columns, titles centered across sections. But the moment one attempts to insert a row, copy a range, or run a macro, the merged cells create operational friction.
...
When a model undergoes audit, consistency is verified by selecting a row and examining for variations. If the FY26 formula differs from FY25, this raises a red flag. It may be intentional — perhaps reflecting a one-time adjustment or step change in assumptions. But the auditor must now determine the reason, and without documentation, the variation remains ambiguous: feature or bug.
...
Consider a model that is inherited. Upon opening, one encounters:
...
Iterative calculation is slow. It's unstable — small changes can cause the model to converge to different values, or fail to converge at all. It's opaque — one can't see the iterations happening, can't verify the logic.

And it disables Goal Seek and Data Tables, two of Excel's most powerful analytical tools.

Better approaches exist: using the RCF as a plug that absorbs the circularity, or employing a User-Defined Function that replicates circular logic in VBA and solves it algebraically in a single pass. These techniques are addressed later. The immediate point: do not permit Excel's iteration engine to solve circularities. They must be solved explicitly and auditably.
...
Avoiding sins is necessary but not sufficient. A model that commits no crimes can still be confusing, poorly organised, and difficult to use. The next sections describe what a high-quality model looks like.
...
- **Inputs**: Assumptions that are subject to change. Growth rates, margins, interest rates, days outstanding. These reside in a dedicated section, clearly labeled, easily located.
...
- **Outputs**: The analytical results of interest. Financial statements, credit metrics, scenario comparisons, sensitivity tables. These appear at the end, summarizing all preceding content.

When a model is opened, assumptions should be immediately locatable. Discovery should not require digging through calculation sheets. Growth assumptions of 3% should be obvious and upfront in the assumptions section.
...
This check should be displayed prominently and formatted to highlight non-zero values. It should be verified continuously during model construction.
...
These checks should be built during model construction. Waiting until the model is "finished" to discover disconnects is inefficient.
...
The flags are then used in formulas:
...
Documentation methods include comments, text boxes, and dedicated "Notes" columns. The specific approach is less important than ensuring reasoning is captured.

When receivable days are assumed at 43, the source should be noted (FDD page 47). When a step-up in interest margin is modeled, the trigger should be noted (leverage below 3.0x per credit agreement section 2.4). When components are excluded from working capital, the rationale should be noted (deferred tax liabilities are non-operating).

Later reviewers — including the original modeler after an elapsed period — will benefit from this documentation.
...
When a managing director asks "What if we increase growth by 50bps?" one can answer in ten seconds, because assumptions are centralised. When an auditor reviews the financing structure, they can follow the logic, because calculations are transparent. When a deal pivots — different structure, different timing, different scenarios — the model flexes, because it was built to flex.

The payoff is confidence: confidence that the model performs as expected, confidence that changes propagate correctly, confidence that the analysis presented aligns with the analysis intended.

This confidence is earned through discipline, through avoiding the sins, and through embracing the principles.

The foundation is established here, before the first formula is written.

---

## When to Go Quarterly: Choosing Your Model's Heartbeat

A question that receives insufficient attention: how frequently should the model update?

Most models default to annual periods. Annual modeling is simpler, requires fewer columns, and produces cleaner layouts. For many purposes, annual is appropriate. When projecting five years forward and examining trajectory, the granularity needed to distinguish March from September may be unnecessary.
...
Annual models reveal the destination. Quarterly models reveal the path taken.
...
If your model only shows year-end figures, one can't assess covenant headroom properly. You might show comfortable compliance at December but have no idea what happened in March, June, or September. The credit agreement does not account for about December if you breached in June.
...
It is common to observe models with monthly sheets for the construction period and separate semi-annual sheets for operations. Models where the debt schedule runs January-December but the revenue build runs April-March. Models where each section has its own timeline, formatted differently, with dates in different rows.
...
A challenge with quarterly models is that annual summaries remain necessary. Credit committees require annual metrics. Covenants frequently test LTM figures. Comparisons with prior years demand annual totals.
...
This hybrid approach provides granularity where essential and simplicity elsewhere.

The transition requires care. Year 2 Q4 must flow cleanly into the Year 3 annual period. Opening balances must align. The LTM calculation for Year 3 must reach back into Year 2's quarterly figures.

Properly structured, the hybrid approach yields the best of both approaches. Poorly structured, the result is two models awkwardly joined, with misalignments at the junctures.

### When Annual Is Fine

Despite the preceding emphasis on quarterly modeling, annual models are often appropriate.
...
Numerous resources teach three-statement financial model construction from first principles. The goal here is not to replicate those approaches but to present a straightforward methodology that captures the specific requirements of leveraged finance modeling.

The model constructed here is intentionally basic. It is readily expandable, refinable, and subject to increasing sophistication. Even in its simplest form, it accomplishes the essential function: characterizing the business through drivers, linking the statements correctly, and enabling capital structure stress-testing.

To make the process memorable, The following acronym is used: **DRIVERS**.
...
Each step is illustrated using Europa Packaging.

---

## D — Data: Arrange Historical Information Into Your Model's Layout

Before modeling can begin, available information must be organized. This principle is self-evident. And yet, a remarkable number of models fail at exactly this step — not due to data inaccuracy, but due to arrangements that compound difficulties in all subsequent steps.

### The Layout Problem

Financial models function as arguments. They embody logic, flow, and narrative structure. Like any effective argument, they require structure.

one can dump three years of audited financials into Excel in about ten minutes. Copy, paste, done. You now have data. What you don't have is a model. You have a mess that happens to contain numbers.
...
It sounds trivial. It's not. It is common to observe models where time flows right to left, models where historical and forecast periods are interleaved, models where 2023 is in column D and 2022 is in column M. These models are not wrong, exactly. They're just unusable.

**Rule 2: Inputs separated from calculations.**

Models contain two types of cells: inputs (assumptions entered by the analyst) and calculations (formulas deriving from inputs and other calculations). These should be visually distinct and, ideally, physically separated.
...
Layouts should reflect the actual business mechanics. For Europa Packaging, this means:
...
For a leveraged finance model, you typically need three to five years of historical data. More is nice but rarely necessary. Less is a problem — one can't identify trends or calculate meaningful averages from two years of history.
...
Information is assembled from multiple sources. Audited accounts provide the totals. The FDD provides component detail. Management presentations provide context.

### Europa Packaging: Setting Up the Layout

To illustrate, the layout development for the case study is shown below.

**Sheet 1: Assumptions / Drivers**

This is where all inputs live. Nothing calculated here — just numbers one can change.
...
A structural consideration is evident: for historical periods (FY21-FY24), the "inputs" are calculated from underlying data — KPIs are being derived. The *structure*, however, is architected so that in the forecast period, these same cells become true inputs: assumptions entered by the analyst.
...
The layout mirrors the driver tree. Revenue is broken by segment. COGS is broken by component. Opex is separated. one can trace any number back to its source.
...
The importance of this principle varies with the analyst's background. For practical purposes, the rule is straightforward: correct linkages yield functional models. Incorrect linkages produce nonsense — balance sheets that fail to balance, cash that materializes from nowhere, profits that dissipate mysteriously.

The linkages must be correct.

### The Core Linkages

There are maybe a dozen connections that matter. Learn these, and one can build any three-statement model.
...
A nuance: interest expense and interest paid are not always identical. When interest accrues but is not paid in cash (PIK interest, for example), the expense affects the income statement without a corresponding cash outflow. For typical models, interest expense may be assumed to equal interest paid.
...
Leveraged finance models require interest expense calculations. Interest expense depends on debt. The cash balance, however, also depends on cash flow. Cash flow depends on interest expense. Interest expense, in turn, depends on...
...
For Europa Packaging, this approach is used. The RCF is the plug. The model calculates operating performance, determines cash needs, and the revolver draws or repays to balance.

### The Linkage Map

The full set of connections is shown below:
...
To illustrate: For Europa's historical periods, most linkages simply organize source data. For the forecast, linkages transform into formulas.
...
In the historical period, it's tempting to just type the numbers. Revenue was €480m, so you type 480. This works — until you realise your historical cash flow statement doesn't tie to your historical balance sheet, and one can't figure out why because nothing is linked.
...
When a circular reference warning appears, the temptation to "fix" it by hard-coding is common. This practice breaks model logic. Legitimate circularities (such as the debt/interest/cash relationship) require either iterative calculation or the plug structure described above. Hard-coding compromises formula integrity.
...
Analysis of historical performance alone provides limited value. The analytical focus shifts to projection and capital structure resilience. This requires moving from *data* to *drivers*. The transition is from "revenue was €480m" to "revenue was €480m because they sold 1.96 billion units at €245 per thousand."
...
Why bother? Because once you understand the components, one can reassemble them with different assumptions. What if volume grows faster? What if prices decline? What if input costs spike? one can't ask these questions of "revenue." one can only ask them of the drivers beneath revenue.
...
The analysis of Europa Packaging demonstrates the approach.
...
This observation reveals a critical business characteristic: Europa Packaging is a volume story, not a pricing story. Growth derives from unit sales expansion rather than price increases. This distinction carries implications for margins, competitive positioning, and downside scenario construction.
...
Has yield been stable? The historical data is set out below:
...
What's your forecast? You probably don't have strong views on resin prices. But one can bracket the range:
...
one can now ask: what if wage inflation runs hot at 4%? What if they cut 5% of headcount through automation? The model can answer.
...
Receivables collection remains stable at 43 days. Customer payment timing has remained consistent. The forecast should maintain 43 days unless evidence suggests otherwise.
...
Every number has a *why* behind it. And once you know the why, one can ask *what if*.
...
A conceptual shift is necessary:
...
It is worth being explicit about what is occurring here.
...
No participant possesses certain knowledge of Europa Packaging's volume growth in FY27. Not the CEO. Not the sponsor. Not advanced analytical systems. The future remains uncertain; the only recourse is making reasonable assumptions about possible outcomes.
...
These assumptions aren't "right." They're *defensible*. one can explain why you chose them. Someone could disagree, and you could have a productive conversation about whether volume growth should be 2.5% or 2.0%.
...
The Europa data is checked below:
...
Having now spent many pages on exactly how to build a model, It bears reaffirmation that warning.
...
Then step back and ask: is the logic sound? Does the model's story align with established about the business, the market, the competitive dynamics?
...
*The next chapter takes this corporate model and transforms it into an LBO model — adding acquisition accounting, a new capital structure, and sponsor economics. Without this foundation, that chapter would be impossible. The LBO model is built on top of the corporate model. Master this first.*
