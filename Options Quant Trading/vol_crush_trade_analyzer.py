...
class VolatilityCrushAnalyzer:
    ...
    def process_market_data(self):
        ...
        if 2 in self.ib_app.historical_data and len(self.ib_app.historical_data[2]) > 0:
            ...
            self.current_iv = normalize_implied_vol(latest_iv, self.iv_unit_mode)
            self.update_status(f"IV normalized with unit mode {self.iv_unit_mode}: {self.current_iv:.1%}")
        else:
