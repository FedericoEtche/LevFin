...
class ImpliedVolatilityDashboard:
    ...
    def query_data(self):
        ...
        if 1 in self.ib_app.historical_data:
            ...
            if len(data) > 0:
                ...
                self.log_message(f"iVol unit mode: {self.iv_unit_mode}")
    ...
    def process_implied_volatility(self):
        ...
        self.log_message(f"Normalizing iVol with unit mode: {self.iv_unit_mode}")
    ...
    def update_current_vol_display(self):
        if self.current_implied_vol is not None:
            ...
            self.vol_computation_label.config(text=f"Unit mode: {self.iv_unit_mode}")
