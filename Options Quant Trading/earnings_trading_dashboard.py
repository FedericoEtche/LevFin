...
from options_core import normalize_implied_vol
import warnings
...
class EarningsTradingDashboard:
    def __init__(self, root):
        ...
        # Historical IB OPTION_IMPLIED_VOLATILITY rows have been stored locally
        # as daily decimal observations in this project. Keep the assumption
        # explicit so it can be changed without touching the analysis code.
    ...
    def analyze_iv_crush(self):
        ...
        if 3 in self.ib_app.historical_data:
            ...
            # Normalize IV data once here. The IB field has appeared in multiple
            # units across saved project files, so the unit mode is explicit.
            raw_iv = iv_data['close']
            iv_data['implied_vol'] = raw_iv.apply(
                lambda value: normalize_implied_vol(value, self.iv_unit_mode)
            )
            self.iv_data = iv_data
            self.log_message(f"Received {len(iv_data)} IV data points - unit mode {self.iv_unit_mode}")
            self.log_message(f"Normalized IV range: {iv_data['implied_vol'].min():.3f} - {iv_data['implied_vol'].max():.3f} (annual decimal)")
        else:
    ...
    def perform_iv_crush_analysis(self):
        ...
        if self.iv_data is not None:
            ...
            # Use the normalized IV data directly.
        ...
        self.log_message(f"IV unit mode: {self.iv_unit_mode}")
